...
The **[Purchase](../docs/purchase.md)** command is typically used for a single-name purchase and returns an immediate success/failure response. Commonly is used by resellers who maintain their own databases.
...
- The domain name to be purchased must be valid \(see requirements under the **[Check](../docs/check.md)** command\).
...
| DomainPassword        | string | Optional; default value is no password                                                           | Set a password on the domain name. For Premium Domains, you cannot set a password in the [Purchase](../docs/purchase.md) command, but you can set a password once the domain name is in your account. |
